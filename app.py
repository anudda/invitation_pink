...
/* 앨범과 정보카드 사이 여백 미세 조정 */
.info-card-container { 
    margin-top: -30px !important; 
}
@media (max-width: 600px) {
    .info-card-container { margin-top: -50px !important; }
}
...
<style>
    ...
    body {{ margin: 0; padding: 0; font-family: 'Gowun Batang', serif; background-color: transparent; }}
    #album-container {{ display: flex; flex-direction: column; align-items: center; gap: 10px; width: 100%; max-width: 450px; margin: 0 auto; }}
    .main-img-wrapper {{
        width: 100%;
        aspect-ratio: 4/5;
...
# PC와 모바일 높이 차이를 극복하기 위해 높이를 조금 더 넉넉하게 잡되, CSS에서 마진을 당김
components.html(album_html, height=580)

# 6. 정보 섹션 (컨테이너로 감싸서 반응형 마진 적용)
st.markdown("""
<div class="info-card-container">
    <div style="background-color: rgba(255, 255, 255, 0.7); padding: 25px 15px; border-radius: 30px; text-align: center; border: 1px solid rgba(255, 143, 171, 0.1);">
...
<div style="display: flex; justify-content: center; gap: 10px; width: 100%; margin-top: 15px;">
    <a href="https://map.kakao.com" target="_blank" style="flex: 1; text-decoration: none; background-color: white; color: #FF8FAB; border: 1.5px solid #FF8FAB; border-radius: 50px; padding: 12px 0; font-family: 'Gowun Batang', serif; font-weight: 700; font-size: 0.85rem; text-align: center;">카카오맵 확인</a>
    <a href="https://map.naver.com" target="_blank" style="flex: 1; text-decoration: none; background-color: white; color: #FF8FAB; border: 1.5px solid #FF8FAB; border-radius: 50px; padding: 12px 0; font-family: 'Gowun Batang', serif; font-weight: 700; font-size: 0.85rem; text-align: center;">네이버 지도 확인</a>
</div>
""", unsafe_allow_html=True)
