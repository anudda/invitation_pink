...
/* 앨범 프레임 설정 */
iframe { width: 100%; border: none; margin-bottom: 0px !important; }
...
<style>
    ...
    .main-wrapper {{ width: 100%; max-width: 450px; display: flex; justify-content: center; }}
    #main-img {{ width: 100%; height: auto; display: block; }}
    .thumb-list {{ display: flex; flex-wrap: nowrap; justify-content: center; align-items: flex-end; gap: 6px; width: 100%; padding: 0 15px; box-sizing: border-box; }}
    ...
    #counter {{ color: #FF8FAB; font-size: 13px; font-weight: bold; margin: 2px 0 10px 0; font-family: sans-serif; }}
</style>
...
// [핵심] 현재 높이를 Streamlit에 전달하는 함수
function sendHeight() {{
    const height = document.getElementById('album-content-wrapper').offsetHeight;
    parent.postMessage({{
...
window.onload = () => {{
    changeImg('{encoded_photos[0]}', 0);
    setTimeout(sendHeight, 500); // 초기 로딩 후 높이 전송
}};
window.onresize = sendHeight;
</script>
"""

# [수정] 앨범 컴포넌트를 호출하고 그 결과값(높이)을 받음
# 초기 높이는 400으로 설정하지만, 안의 JS가 실제 높이로 덮어씌웁니다.
h = components.html(album_content, height=450)

# 6. 정보 섹션 (마이너스 마진 없이 깔끔하게 배치)
st.markdown("""
<div style="background-color: rgba(255, 255, 255, 0.8); padding: 25px 15px; border-radius: 35px; text-align: center; border: 1px solid rgba(255, 143, 171, 0.12); box-shadow: 0 10px 20px rgba(255, 143, 171, 0.05); margin-top: 5px;">
